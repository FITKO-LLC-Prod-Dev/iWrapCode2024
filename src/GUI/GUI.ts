...
class GUI {
  ...
  constructor(container: HTMLElement, menuItems: TMenuItem[]) {
    ...
    menuItems.forEach((item) => {
      if (item.type === SupportedMenuItemTypes.Button)
        this.addMenuButton(item.text, item.id, item.onClick);
      else if (item.type == SupportedMenuItemTypes.InputSpinner)
        this.addMenuInputSpinner(
          item.id,
          item.defaultOptionIdx,
          item.options,
          item.onChange,
        );
    });
  ...
  private addMenuInputSpinner(
    ...
    arrowLeft.addEventListener("click", (_: MouseEvent) => {
      ...
      if (optionsIdx - 1 > 0) {
        --optionsIdx;
        onChange(options[optionsIdx]);
        label.textContent = options[optionsIdx].toUpperCase();
        return;
      }
    ...
    arrowRight.addEventListener("click", (_: MouseEvent) => {
      ...
      if (optionsIdx + 2 < options.length) {
        ++optionsIdx;
        onChange(options[optionsIdx]);
        label.textContent = options[optionsIdx].toUpperCase();
        return;
    ...
    menuItem.appendChild(spinnerDiv);
    this.startMenu.appendChild(menuItem);
    return spinnerDiv;
  }
}

export { GUI, SupportedMenuItemTypes, TMenuItem };
